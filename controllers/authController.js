...
const getMe = async (req, res) => {
    try {
        ...
        try {
            [rows] = await db.query(
                'SELECT id, name, email, username, role, user_type, status, canAccessRefueling, canAccessAnaliseGerencial, bloqueado_abastecimento, page_permissions, can_create_hidden_orders, employeeId FROM users WHERE id = ?',
                [req.user.id]
            );
        } catch (colErr) {
            [rows] = await db.query(
