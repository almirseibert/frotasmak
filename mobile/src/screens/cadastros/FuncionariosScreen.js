...
const statusPill = (status = '') => {
  ...
  if (t.includes('afast')) return { label: status, bg: '#ffedd5', text: '#9a3412', dot: '#f97316' };
  if (t.includes('inativ') || t.includes('deslig')) return { label: status, bg: '#f3f4f6', text: '#6b7280', dot: '#9ca3af' };
...
export default function FuncionariosScreen() {
  ...
  const filtered = useMemo(() => {
    ...
    return rows.filter((e) =>
      [e.nome, e.vulgo, e.funcao, e.registroInterno, e.cidade]
        .filter(Boolean)
    ...
    <View style={{ flex: 1, backgroundColor: colors.appBg }}>
      <View style={s.searchWrap}>
        ...
        <TextInput
          ...
          placeholder="Nome, função ou registro…"
      ...
      <FlatList
        ...
        ListEmptyComponent={<EmptyState icon="account-off-outline" title="Nenhum funcionário encontrado" />}
        renderItem={({ item }) => {
          const st = statusPill(item.status);
          return (
            <View style={s.item}>
              <View style={s.itemIcon}>
                <Icon name="account" size={20} color={colors.amber} />
              </View>
              <View style={{ flex: 1, minWidth: 0 }}>
                <View style={{ flexDirection: 'row', alignItems: 'center', gap: 6 }}>
                  <Text style={s.itemTitle} numberOfLines={1}>
                    {item.nome}{item.vulgo ? ` (${item.vulgo})` : ''}
                  </Text>
                  <Pill label={st.label} bg={st.bg} text={st.text} dot={st.dot} />
                </View>
                <Text style={s.itemMeta} numberOfLines={1}>
                  {[item.funcao, item.alocacaoAtual, item.cidade].filter(Boolean).join(' · ') || '—'}
                </Text>
              </View>
            </View>
          );
